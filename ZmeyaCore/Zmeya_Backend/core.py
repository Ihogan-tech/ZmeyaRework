...
def customScan(sPaths=None, lPath=None, qPath=None):
    if sPaths:
    ...
    retval = basicScan(sPaths, lPath, qPath)
    print(lock(qPath))
    return retval
...
def basicScan(sPaths, lPath=None, qPath=None):
    ...
    #Command for ClamAV
    scan_command = [os.path.dirname(os.path.realpath(__file__))+"\\ClamAV\\clamscan.exe", "-r", "--max-dir-recursion=10",f"--move={qPath}", "-i"]
...
def fullScan(lPath=None, qPath=None):
    """
...
#customScan(["C:/Users/kitty/Desktop/scanMe"])
